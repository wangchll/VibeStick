...
此功能是状态栏“语音识别模式”的第三个选项，不会替代云端 API 或本地 Whisper。切换模式后 Bridge 会重启并读取新配置。

此模式在 StickS3 录音期间持续上传 PCM，并实时写入 macOS 虚拟音频设备；
同时模拟长按微信输入法默认的 Fn 语音快捷键。微信输入法直接把识别结果写入
...
3. 正常使用时直接在 VibeStick 状态栏选择“微信语音输入法（长按 Fn）”。以下 `.env` 设置仅用于排查或手工开发：
...
录音实际送达 BlackHole 后，再保留一小段尾部静音并松开。备用的 `toggle`
模式仍支持 `control`、`option`、
...
若扬声器实际发声，说明系统音频路由或多输出设备配置不正确；检查默认输出没有选择 BlackHole 聚合/多输出路径，并重新运行安装器。
...
- Bridge 在录音开始时启动常驻 helper 并按下 Fn。
- StickS3 以带字节偏移的 PCM 分块持续上传；重复请求不会重复输入音频。
- 松开按键时只发送最后一块，Bridge 等 BlackHole 播放队列排空后松开 Fn。
- 如果 Bridge、helper 或网络不支持本次流式会话，固件会回退到松开后上传
  完整录音，保证旧版本兼容和录音可恢复。

## 当前限制

- Wi-Fi 抖动可能让 BlackHole 队列短暂欠载；完整 PCM 仍在 Bridge 端合并为
  WAV，流式失败时会自动使用整段录音回退。
