...
# VibeStick ships only the unencrypted `generate` path. Keep the upstream
# encryption commands available when cryptography is installed, but do not make
# that optional dependency a requirement for normal consumer installation.
try:
    from cryptography.hazmat.backends import default_backend
    from cryptography.hazmat.primitives import hashes, hmac
    from cryptography.hazmat.primitives.ciphers import Cipher, algorithms, modes
except ImportError:
    default_backend = hashes = hmac = Cipher = algorithms = modes = None
...
def main():
    ...
    if len(sys.argv) <= 1:
        ...
        if args.command != 'generate' and default_backend is None:
            parser.error('cryptography is required for encrypted NVS operations')
        args.func(args)
